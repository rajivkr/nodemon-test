...
var monitor = function(options) {
  ...
  function site(req, res) {
    ...
    var customHealthCheck = healthCheck();
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.write(customHealthCheck);
    res.write(cpuData);
  ...
  /*
   * Functions to create custom health check response
   * TODO: Add health check on services once they are added as dependencies
   */
  function healthCheck() {
    var content = '';
    content += '# CUSTOM HEALTH CHECK\n';
    content += 'health_check 1\n';
    return content;
  }
  ...
  function stringMemoryData() {
    ...
    content +=
      'os_resident_memory_bytes ' + latestMemEvent.physical_used + '\n';
    content +=
      '# HELP process_resident_memory_bytes Resident memory size in bytes.\n';
    content += '# TYPE process_resident_memory_bytes gauge\n';
    content +=
      'process_resident_memory_bytes ' + latestMemEvent.physical + '\n';
    content +=
  ...
  monitoring.on('http', function(data) {
    ...
    while (found === false && i < httpURLDataList.length) {
      if (
        httpURLDataList[i].url == data.url &&
        httpURLDataList[i].code == data.statusCode
      ) {
        found = true;
        foundIndex = i;
      }
    ...
    if (found) {
      ...
      urlData.duration_list.sort(function(a, b) {
        return a - b;
      });
    } else {
      httpURLDataList.push({
        url: data.url,
